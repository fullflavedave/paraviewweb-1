...
/***/ function(module, exports, __webpack_require__) {
	...
	var _CompositeClosureHelper = __webpack_require__(169);
	...
	function legendHelper(publicAPI, model) {
	  publicAPI.addName = function (name) {
	  ...
	  publicAPI.removeName = function (name) {
	    if (model.names.indexOf(name) !== -1 && name) {
	      model.names.splice(model.names.indexOf(name), 1);
	      model.dirty = true;
	    }
	  };
	  publicAPI.removeAllNames = function () {
	    model.names = [];
	    model.dirty = true;
	  };
	  ...
	  publicAPI.assignLegend = function () {
	    ...
	    if (model.dirty) {
	      (function () {
	        var shapesArray = Object.keys(model.shapes);
	        model.dirty = false;
	        model.legendMapping = {};

	        if (model.priorities && model.priorities.length) {
	          (function () {
	            var defaultColor = model.colors[0];
	            var defaultShape = shapesArray[0];

	            var iterator = createSortedIterator(model.priorities, { colors: model.colors, shapes: shapesArray }, { colors: defaultColor, shapes: defaultShape });
	          ...
	          model.names.forEach(function (name, idx) {
	  ...
	  publicAPI.usePalette = function (name) {
	  ...
	  publicAPI.updateSettings = function (settings) {
	    ['shapes', 'colors', 'names', 'priorities'].forEach(function (key) {
	      if (settings[key]) {
	  ...
	  publicAPI.listPalettes = function () {
	...
	function extend(publicAPI, model) {
	  ...
	  _CompositeClosureHelper2.default.isA(publicAPI, model, 'LegendHelper');
