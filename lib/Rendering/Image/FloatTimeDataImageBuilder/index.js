...
var _LineChartPainter = require('../../Painter/LineChartPainter');
...
var FloatTimeDataImageBuilder = function () {
  ...
  function FloatTimeDataImageBuilder(floatDataImageBuilder, timeProbeManager) {
    var _this = this;
    ...
    this.activeView = 0;
    this.subscriptions = [];
    this.painter = new _LineChartPainter2.default('');
      ...
      switch (this.activeView) {
        ...
        case 1:
          if (this.renderer && this.renderer.getRenderingCanvas && this.painter.isReady()) {
            var canvas = this.renderer.getRenderingCanvas();
            var width = canvas.width;
            var height = canvas.height;

            var ctx = canvas.getContext('2d');
            var offset = 5;
            var location = { x: offset, y: offset, width: Number(width) - 2 * offset, height: Number(height) - 2 * offset };
