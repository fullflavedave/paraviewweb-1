  ...
  function ColorByHelper(layers, fieldCodes, lookupTableManager) {
    ...
    for (var layerIdx = 0; layerIdx < this.nbLayers; layerIdx++) {
      ...
      while (count) {
        ...
        var colorName = colorBy.name;
        var lut = this.lookupTableManager.getLookupTable(colorBy.name);
    ...
    value: function updateData(data) {
      ...
      Object.keys(data).forEach(function (name) {
        if (name.indexOf('_') !== -1) {
          ...
          var layerName = encoding[Number(splitName.shift())];
          var colorBy = splitName.join('_');
