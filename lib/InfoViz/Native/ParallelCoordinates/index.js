...
exports.createParallelCoordinates = createParallelCoordinates;
function affine(inMin, val, inMax, outMin, outMax) {
  return (val - inMin) / (inMax - inMin) * (outMax - outMin) + outMin;
}

function createParallelCoordinates(canvasElement, fetchHistogramFunction, selectionQueryFunction, selectionClearedCallback) {
  ...
  function sumCheckHistograms(hLeft, hRight) {
    ...
    Object.keys(leftBinMap).forEach(function (leftKey) {
      if (leftBinMap[leftKey] !== rightBinMap[leftKey]) {
        console.error('Sanity check failed for bin:', leftKey);
      }
    });

    Object.keys(rightBinMap).forEach(function (rightKey) {
  ...
  function getMaxHistogramBinCount(histoList) {
    ...
    for (i = 0; i < histoList.length; ++i) {
      ...
      histogram.bins.map(function (currentValue, index, array) {
        if (currentValue.count > _this.max) {
          _this.max = currentValue.count;
          return true;
        }
        return false;
      }, result);
  ...
  function drawAxes() {
    var i, j, annList, dataRange, axisRightSideUp, ann;
  ...
  function receivedHistogram(histIdx, histogram) {
    ...
    if (histogramCount === axisList.length - 1) {
      // We have received all histograms
  ...
  function mouseHandler(mouseEvent) {
    ...
    if (action === 'mouseup') {
      ...
      // must be a move event, ignore if not dragging...
      if (dragging) {
